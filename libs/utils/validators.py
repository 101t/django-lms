...
def isValidHTML(field_data, all_data):
	...
	import urllib.parse
	try:
		u = urllib.request.urlopen('http://validator.w3.org/check',
		                    urllib.urlencode({'fragment': field_data, 'output': 'xml'}))
	except:
	...
	error_messages = [e.firstChild.wholeText for e in
	                  parseString(u.read()).getElementsByTagName('messages')[0].getElementsByTagName('msg')]
...
def isValidUSState(field_data, all_data):
	...
	states = ['AA', 'AE', 'AK', 'AL', 'AP', 'AR', 'AS', 'AZ', 'CA', 'CO', 'CT', 'DC', 'DE', 'FL', 'FM', 'GA', 'GU',
...
class AlwaysMatchesOtherField(object):
	def __init__(self, other_field_name, error_message=None):
		...
		self.error_message = error_message or lazy_inter(gettext_lazy("This field must match the '%s' field."),
		                                                 self.other)
...
class RequiredIfOtherFieldsGiven(object):
	def __init__(self, other_field_names,
	             error_message=gettext_lazy("Please enter both fields or leave them both empty.")):
...
class RequiredIfOtherFieldEquals(object):
	def __init__(self, other_field, other_value, error_message=None, other_label=None):
		...
		self.error_message = error_message or lazy_inter(
			gettext_lazy("This field must be given if %(field)s is %(value)s"), {
...
class RequiredIfOtherFieldDoesNotEqual(object):
	def __init__(self, other_field, other_value, other_label=None, error_message=None):
		...
		other_label = other_label or other_value
		self.error_message = error_message or lazy_inter(
			gettext_lazy("This field must be given if %(field)s is not %(value)s"), {
				'field': other_field, 'value': other_label})
		self.always_test = True
...
class NumberIsInRange(object):
	...
	def __init__(self, lower=None, upper=None, error_message=''):
		...
		if not error_message:
			if lower and upper:
				self.error_message = gettext("This value must be between %(lower)s and %(upper)s.") % {'lower': lower,
				                                                                                       'upper': upper}
			elif lower:
	...
	def __call__(self, field_data, all_data):
		...
		if '.' in data and len(data.split('.')[1]) > self.decimal_places:
			raise ValidationError(ngettext("Please enter a valid decimal number with at most %s decimal place.",
			                                ...
			                                self.decimal_places) % self.decimal_places)
...
class HasAllowableSize(object):
	...
	def __init__(self, min_size=None, max_size=None, min_error_message=None, max_error_message=None):
		...
		self.min_error_message = min_error_message or lazy_inter(
			gettext_lazy("Make sure your uploaded file is at least %s bytes big."), min_size)
		self.max_error_message = max_error_message or lazy_inter(
			gettext_lazy("Make sure your uploaded file is at most %s bytes big."), max_size)
...
class URLMimeTypeCheck(object):
	...
	def __call__(self, field_data, all_data):
		...
		if content_type not in self.mime_type_list:
			raise URLMimeTypeCheck.InvalidContentType(gettext(
