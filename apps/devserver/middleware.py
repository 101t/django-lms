from apps.devserver.models import MODULES
...
class DevServerMiddleware(object):
	def should_process(self, request):
		...
		if getattr(settings, 'STATIC_URL', None) and request.build_absolute_uri().startswith(
				request.build_absolute_uri(settings.STATIC_URL)):
			return False

		if settings.MEDIA_URL and request.build_absolute_uri().startswith(
	...
	def process_init(self, request):
		from apps.devserver.utils.stats import stats
