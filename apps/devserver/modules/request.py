...
class RequestDumpModule(DevServerModule):
	...
	def process_request(self, request):
		...
		request.method, '?'.join((request.META['PATH_INFO'], request.META['QUERY_STRING'])),
		...
		if request.POST:
			req += '\n%s\n' % self.logger.style.HTTP_INFO(
				urllib.urlencode(dict((k, v.encode('utf8')) for k, v in request.POST.items())))
		if request.FILES:
