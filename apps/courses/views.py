...
class CourseOverview(DetailView):
	...
	def dispatch(self, request, *args, **kwargs):
		...
		if course.private:
			if request.user not in itertools.chain(course.faculty.all(), course.members.all(),
			                                       course.teaching_assistants.all()) and not request.user.is_superuser:
...
class AssignmentOverview(DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		if self.request.user in self.get_object().course.faculty.all():
			...
			context['submissions'] = AssignmentSubmission.objects.filter(users=self.request.user,
...
class SubmitAssignment(CreateView):
	...
	def form_valid(self, form):
		...
		if self.object.file:
			statsd.incr("courses.{}.{}.assignments.storage".format(self.object.assignment.course,
			                                                       self.object.assignment.course.semester,
...
class TeamSubmitAssignment(SubmitAssignment):
	...
	def get_form(self, form_class):
		...
		form.fields['users'].queryset = User.objects.filter(
			id__in=[user.id for user in self.assignment.course.members.all()])
...
class CourseCalendar(TemplateView):
	...
	def get_context_data(self, **kwargs):
		...
		for dt in rrule.rrule(rrule.MONTHLY, dtstart=semester.start, until=semester.end):
			calendars.append(
				HTMLCourseCalendar(events, user_cal=kwargs.get('user_cal', False)).formatmonth(dt.year, dt.month))
