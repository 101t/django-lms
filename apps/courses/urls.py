...
urlpatterns = [
	...
	url('^(?P<pk>\w+)/assignments/team_submit/$', login_required(TeamSubmitAssignment.as_view()),
	...
	url('^calendar/(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)/$', login_required(CourseCalendarDay.as_view()),
	    name='calendar_day'),
	...
	url('^user_calendar/(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)/$', login_required(UserCourseCalendarDay.as_view()),
